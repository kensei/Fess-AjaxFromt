$(function(){
  // (1) Fess の URL
  var baseUrl = "http://search.n2sm.co.jp/fess/json?callback=?&query=";
  ...
  var doSearch = function(event){
    if (event.type == 'popstate') {
      var popurl = $.url();
      $('#searchStart').val(popurl.fparam('start'));
      $('#searchQuery').val(popurl.fparam('query'));
    }
      ...
      $.ajax({
        ...
        complete: function() {
          if (window.history && event.type != 'popstate') {
            var pushurl = "#" + $("form").serialize();
            if ($(this).attr('href')) {
              pushurl = $(this).attr('href') + pushurl;
            }
